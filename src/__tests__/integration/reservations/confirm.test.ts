...
beforeAll(async () => {
  ...
  const d1 = await prisma.donation.create({
    data: { status: 'RESERVED', token: 'ABC123', donorId: donor.id, ongId: ong.id },
  })
  donationId = d1.id

  const d2 = await prisma.donation.create({
    data: { status: 'AVAILABLE', token: null, donorId: donor.id },
  })
  ongDonationId = d2.id

  const d3 = await prisma.donation.create({
    data: { status: 'COLLECTED', token: null, donorId: donor.id, ongId: ong.id },
  })
