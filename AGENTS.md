...
-   *(V1.5 - 23/09/2026)*: **Prefetch de `<Link>` para rotas API desloga usuários**: o Next.js App Router perfetches links visíveis mesmo em Server Components/Actions. O `<Link href="/api/v1/auth/logout">` sem `prefetch={false}` disparava um `GET ?_rsc=` no route de logout logo após o login, limpando o cookie `auth_token` e causando 401 "Não autorizado." em todas as chamadas seguintes (sintoma típico: usuário loga mas não consegue criar/ver nada). Regra: **todo `<Link>` apontando para `/api/*` deve ter `prefetch={false}`** (padrão já aplicado nos links do dashboard em V1.3).
